...
export default function Search() {
  ...
  useEffect(() => {
    ...
    const storedMatchedAccounts = JSON.parse(localStorage.getItem('matchedAccounts'));
  ...
  }, [searchTerm, selectedInstrument, selectedSubcategory, selectedGenre, selectedSkillLevel, matchedAccounts]);
  ...
  const handleSubmit = () => {
    ...
    const matches = fakeAccounts.filter((account) => {
      return (
        account.instrument === selectedInstrument.name &&
        (!selectedSubcategory || account.subcategory === selectedSubcategory) &&
        (!selectedGenre || account.genre === selectedGenre) &&
        (!selectedSkillLevel || account.skillLevel === selectedSkillLevel)
      );
